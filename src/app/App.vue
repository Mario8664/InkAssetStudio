  ...
  <div class="studio-shell" :class="{ loading }">
    ...
    <footer v-if="session.mode === 'draw'" class="tooltray">
      ...
      <div class="tool-group tool-options">
        ...
        <button class="toggle" :class="{ active: session.pressureEnabled }" title="Use Apple Pencil pressure for new Outline points, Fill Brush or Blur size, Fill Eraser size, and Water opacity" @click="session.pressureEnabled = !session.pressureEnabled">Pressure {{ session.pressureEnabled ? 'On' : 'Off' }}</button>
